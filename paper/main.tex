...
\begin{table}[t]
\centering
\input{guard_scope_stress_table.tex}
\caption{V2 guard-scope stress. Exact guard scope is the central assumption: under-scoped repair misses false shortcuts, while over-scoped repair preserves success at higher search cost.}
\label{tab:guard-scope}
\end{table}
...
\section{Discussion}
The strongest claim here is not that CCRA is the final answer for robotics. It is that recoverability is a different mechanism from prediction. When a robot discovers that the world contradicts a planned transition, the system should have a way to rewrite what future plans are allowed to assume. Table~\ref{tab:guard-scope} shows the main boundary: this rewrite is only useful when the guard is scoped well enough to catch repeated false affordances without blocking too many valid alternatives.
...
\section{Limitations}
The evidence is synthetic and the formal lemma is narrow. The v2 stress also shows that calibrated guard scope is a hard requirement: under-scoped patches lose success and over-scoped patches increase search cost. The paper does not establish real-robot scaling, perception robustness, or fully automatic guard learning. Those limitations are deliberate. The point is to isolate a mechanism that existing prediction-centric framing tends to hide.

\section{Conclusion}
Robotic plan recoverability should be treated as a first-class mechanism. CCRA is a simple way to do that: use execution counterexamples to patch the planner-facing transition relation. The v2 hardening result keeps the claim modest: planner-facing repair helps in the toy only when guard scope is reliable. The literature sweep suggests this is still a meaningful boundary from current robot planning, world modeling, and adaptation work.
